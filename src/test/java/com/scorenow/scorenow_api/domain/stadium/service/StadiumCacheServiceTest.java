...
class StadiumCacheServiceTest {
    ...
    void 캐시와_DB에_존재하지_않는_경기장인_경우_DB에_저장_후_캐시에도_저장() {
        ...
        // 캐시에 존재하지 않는다.
        Stadium beforeCached = stadiumCache.getIfPresent(new StadiumCacheKey(ExternalProvider.BETS, externalSportId, externalStadiumId));
        assertThat(beforeCached).isNull();

        // Mapping Table에 존재하지 않는다.
        Optional<StadiumExternalMapping> stadiumMappingInfo = stadiumExternalMappingRepository.findByExternalInfo(ExternalProvider.BETS, externalSportId, externalStadiumId);
        assertThat(stadiumMappingInfo.isEmpty()).isTrue();

        // DB 및 캐시에 저장
        Stadium savedStadium = stadiumCacheService.getOrCreateStadium(ExternalProvider.BETS, externalSportId, externalStadiumId, externalName, externalCity);

        // 캐싱 되었는지 검증
        assertThat(stadiumCache.getIfPresent(new StadiumCacheKey(ExternalProvider.BETS, externalSportId, externalStadiumId)))
                .isNotNull();
    ...
    void 캐시에는_없지만_DB에_존재하는_경기장인_경우_DB에서_조회_후_캐시에_저장() {
        ...
        // DB 에 미리 저장한다.
        Stadium savedStadium = stadiumService.getOrCreateStadium(ExternalProvider.BETS, externalSportId, externalStadiumId, externalName, externalCity);

        // 검증 대상 메서드 호출
        stadiumCacheService.getOrCreateStadium(ExternalProvider.BETS, externalSportId, externalStadiumId, externalName, externalCity);
    ...
    void 캐시에_있는_경기장인_경우_DB에서_조회하거나_저장하는_로직이_실행되지_않는다() {
        ...
        StadiumExternalMapping stadiumExternalMapping = StadiumExternalMapping.of(ExternalProvider.BETS, externalSportId, externalStadiumId, internalSportId);
