...
public class TeamService {
	 ...
	 */
	@Transactional
	public Team getOrCreateTeam(final ApiProvider provider, final Long internalSportId, final String apiTeamId,
		...
		if (teamMappingInfo.isPresent()) {
			...
			return teamRepository.findById(internalTeamId)
				.orElseGet(() -> createAndMapTeam(provider, internalSportId, apiTeamId, name, cc, imageUrl));
		}

		return createAndMapTeam(provider, internalSportId, apiTeamId, name, cc, imageUrl);
	}

	private Team createAndMapTeam(ApiProvider provider, Long internalSportId, String apiTeamId, String name,
